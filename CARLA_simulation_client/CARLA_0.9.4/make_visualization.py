...
timestamp="2020-03-01-21-57-41.449517" # Timestamp of run to visualize
path="/some_results/" # Path to settings files from the drive
env_path="Datasets/carla-sync/val/" # Path to the environment files from the drive
# Path to statistics files
stat_path="Results/statistics/val/" if USE_LOCAL_PATHS == 0 else "localUserData/Results/statistics/val/"
...
for scene in scenes:
    ...
    print(filename_list[int(nbr/4)] + " " + str(nbr))
    if int(nbr)==32 and int(epoch) ==8:
        ...
        print("Agent positions: "+str(init_pos)+" "+str(agent_pos[1,:]))
        ################################################################
        agent_positions=[]
        agent_yaw=[]
        previous_pos=[0,0]
        p = np.array([0, agent_goals[0], agent_goals[1]]) # Point in my coordinate system
        ...
        print("goal " + str(p))
        agent_goal = p # The same point in CARLA coordinate system
        directions=[]
        directions_angle=[]
        ...
        for frame in range(agent_pos.shape[0]):
            if reached_goal[max(frame-15,0)]==0:
                ...
                #print pfnn_itr
                pfnn_pos = agent_pfnn_pos[pfnn_itr, :]
                pfnn_pose = agent_poses[pfnn_itr, :] # Agent pose in PFNN coordinate system
                #print pfnn_pose

                z = pfnn_pos[1]
                x = pfnn_pos[0]
                current_pos = np.array([z, x]).transpose()

                pos_exact=copy.copy(init_pos)
                target_rotated = np.matmul(inverse_rotation_matrix, current_pos)
                pos_exact[1] = target_rotated[0] / 20.0 + init_pos[1]
                pos_exact[2] = target_rotated[1] / 20.0 + init_pos[2]

                position = np.round(pos_exact).astype(int) # PFNN position in my coordinate system
                ##################################################################################################
        ...
        try:
            ...
            while my_frame<len(agent_positions):
                ...
                if not actor_perspective:
                    image.save_to_disk(os.path.join(out_path, 'test_%02d_%02d_%06d.jpg' % (int(nbr),int(epoch),my_frame)))
                    name_movie_main=os.path.join(out_path,'test_%02d_%02d' % (int(nbr),int(epoch)))
                    name_movie =name_movie_main+'_%06d.jpg'
                else:
                    image.save_to_disk(os.path.join(out_path,'test_perpective_%02d_%02d_%06d.jpg' % (int(nbr), int(epoch), my_frame)))
                    name_movie_main = os.path.join(out_path,'test_perpective_%02d_%02d' % (int(nbr),int(epoch)))
                    name_movie = name_movie_main + '_%06d.jpg'
