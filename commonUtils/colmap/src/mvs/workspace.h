...
struct Options {
    ...
    std::string seg_map_path;//"/home3/GRFP/results";
    std::string seg_map_path_right;//"/home3/GRFP/results_right";
  };
  ...
  const Bitmap& GetSegmentation(const int image_id);
  ...
  std::string GetSegmentationPath(const int image_id) const;
  ...
  bool HasSegmentation(const int image_id) const;
 ...
 private:
  ...
  class CachedImage {
   public:
    ...
    std::unique_ptr<Bitmap> segmentation;
  ...
  std::string seg_map_path_;
  std::string seg_map_path_right_;
};
