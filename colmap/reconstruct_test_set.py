...
def get_camera_matrices(city, seq_nbr, mode_name, frame_nbr='000019', nbr_frames=30):
    ...
    frames = list(range(frame_nbr-19, frame_nbr-19+nbr_frames))
...
def write_img_list(path, city, seq_nbr,  cam_loc_left, camera_loc_right , frame_nbr,nbr_frames):
    plot = False


    img_list = open(path+"/list_images.txt", 'w')

    for frame in range(frame_nbr-19, frame_nbr-19+nbr_frames):
    ...
    for city in ['bochum', 'hamburg', 'hanover', 'krefeld', 'monchengladbach', 'strasbourg']:
        for filename in glob.glob(const.gt_path+mode_name+'/'+city+'*/*.png'):
            ...
            if const.label_name in filename:  # 'gtFine_labelIds.png' in file:
                ...
                seq_nbr = parts[1]
                frame_nbr = parts[2]
                frame_nbr_i=int(frame_nbr)
                path='Datasets/colmap/'+city+'_'+seq_nbr+'_'+frame_nbr
                ...
                for frame_s in range(frame_nbr_i - 19, frame_nbr_i + 11):
                    img_file_name='{:0>2d}'.format(frame_s)
                    right_img_path=os.path.join(const.right_path,  mode_name ,city ,city+'_' + seq_nbr +'_' +img_file_name+'*')
                    print('cp ' + right_img_path +' '+ path + '/images/right/')
                    status = subprocess.call('cp ' + right_img_path +' '+ path + '/images/right/',
                        shell=True)
                if not os.path.exists(path+'/images/left'):
                     os.mkdir(path + '/images/left')
                for frame_s in range(frame_nbr_i-19,frame_nbr_i+11):
                    img_file_name = '{:0>2d}'.format(frame_s)
                    right_img_path = os.path.join(const.img_path, mode_name, city,
                                                  city + '_' + seq_nbr + '_' + img_file_name + '*')
                    print('cp ' + right_img_path + ' ' + path + '/images/left/')
                    status = subprocess.call('cp ' + right_img_path + ' ' + path + '/images/left/',shell=True)
                ...
                for frame_s in range(frame_nbr_i - 19, frame_nbr_i + 11):
                    img_file_name = '{:0>2d}'.format(frame_s)
                    right_img_path = os.path.join(segmentation_path,
                                                  city + '_' + seq_nbr + '_' + img_file_name + '*')
                    print('cp ' + right_img_path + ' ' + path + '/segmentation/left/')
                    status = subprocess.call('cp ' + right_img_path + ' ' + path + '/segmentation/left/', shell=True)

                    left_img_path = os.path.join(segmentation_path_right,
                                                  city + '_' + seq_nbr + '_' + img_file_name + '*')
                    print('cp ' + left_img_path + ' ' + path + '/segmentation/right/')
                    status = subprocess.call('cp ' + left_img_path + ' ' + path + '/segmentation/right/', shell=True)
