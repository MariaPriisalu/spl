...
if __name__ == "__main__":
    ...
    for dirname in glob.glob('Datasets/colmap/colmap/*'):
        # If this is an image.
        ...
        if len(parts)>1:
            ...
            seq_nbr = parts[1]
            skip = False
            if city in ['bochum', 'hamburg', 'hanover', 'krefeld', 'monchengladbach', 'strasbourg']:
                skip = True
            if 'darmstadt_000033' in dirname:
                start=True
                skip=True

            path='Datasets/colmap/colmap/'+city+'_'+seq_nbr


            if start and not skip and (os.path.isfile(path+'/sparse/0/cameras.bin') or os.path.isfile(path+'/sparse/cameras.bin')) :
                print(path)
                ...
                status = subprocess.call(
                    ...
                    '--input_path ' + path + '/sparse/0 --output_path ' + path + '/sparse/0 --rig_config_path Datasets/colmap/colmap/rig_camera.json',
                    shell=True)
                ...
                print('Packages/colmap/colmap/build/src/exe/colmap dense_fuser --workspace_path ' + path + '/dense  --workspace_format COLMAP --input_type geometric --output_path ' + path + '/dense/fused.ply --output_path_seg ' + path + '/dense/fused_seg.ply  --output_path_seg_col '+  path + '/dense/fused_seg_col.ply') #--output_path_text ' + path + '/dense/fused_text.ply
                status = subprocess.call(
            ...
            number_recons += 1
            print("Reconstruction done for " + str(number_done) + "Out of " + str(number_recons) + " " + str(
